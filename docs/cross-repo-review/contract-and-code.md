...
本页是双向实现导航。消费者的正式调用定义仍在[输入输出协议](../input-output.md)，生产侧见[交付说明](../../../REVISE/docs/development/reconstruction-analysis/plans/analysis-handoff.md)。核对日期为 2026-09-19；行号只作该工作树定位提示，函数名优先。
...
| 表达接口 | 已确认线性来源经数值检查后输出 matrix=X、identity；未知来源保留限制 | 固定 finite/nonnegative/unlogged linear；允许小数及小值；旧 log 声明拒绝，identity unknown 或旧 scale unknown 阻止表达消费者 | `expression.consumer_declaration` 对已知来源不写旧 scale；不表示历史 P2 已获确认。loader 不猜历史，数值检查在表达方法执行 |
| 标签 | 交付映射指向推断 broad/subtype；SVC 实际有 `SVC_cluster` 才声明 reconstruction | 使用映射与视图别名；State 使用输入主重建标签，Raw Level2 缺失不补算 | 主重建分群不等于已注释 biological subtype；可读取不代表每条 route 都具备 State 所需标签 |
...
| 空间坐标 | 检查两侧声明坐标形状及有限性，um 转 micron；未知比例不猜 | 物理窗口按声明单位与转换比例判断；Anatomy 与 State 独立尺度 | sST 当前发布虚拟单位坐标；synthetic 覆盖不等于真实 sST 空间验收 |
...
四种方法在共同范围内各自进行表达预处理及 Leiden；旧空间标签不用于生成新表达分群。保留多 resolution、覆盖表、列联表及 ARI/NMI，不自动挑赢家。历史 T/Macro/CAF 三个标签载体的来源已确认，但 whole-sample 比较基准整合和四种真实方法产物仍待准备。本轮不复制该实验，也不将其局部配对规则写入普通 loader。
