...
确认 Raw 的表达 identity/scale 后，Raw baseline、Gain、Raw K-control 和 Raw-defined membership 可按各自的标签、空间与共同 ID 前提运行。Moran 与 AUCell 每侧独立：一侧表达条件满足即可保存该侧结果。跨侧比较（shared-gene Moran、Raw/SVC 共同窗口差值等）需要比较涉及的两侧条件都满足；否则保留单侧证据并明确比较不可用。非负线性浮点值（包括很小的值）可直接作为输入，流程不做隐式四舍五入。
...
State 对 SVC Neff 选择稳定阈值。主差值在共同有效窗口保存全部正负 ΔNeff，仅正差值进入 Gain candidate 阈值。沿用最少 200 个值、500 次 bootstrap、有效 bootstrap 至少 80%、95% 区间宽度不超过观测范围 25% 的规则。失败仍保存连续场、分布、支持、bootstrap 和原因；不把失败转成任意分位数或空的“成功区域”。
...
Integrated Spatial Evidence 汇总 State/Gain × Anatomy、State 内外与 Anatomy 中主标签组成、program × window/Anatomy/Region、changed-unit × Region/Anatomy，并指回底表。标签组成用于解释多样性，不作为独立验证。无需合并 Raw/SVC 表达矩阵。

## 共用执行与缺失

Notebook 和 batch 使用 `effective_parameters` 与 `ImpactWorkflow` 同一分阶段流程。样本正式参数先加载，再显式 override；Notebook 显示实际配置和中间对象，不在样本路径错误时退回 example。

每侧声明 expression identity/scale。unknown 允许标签空间分析，但表达消费者保持 unavailable。已确认线性非负表达先归一化和 log1p，已确认 log1p 不重复变换；HVG 使用 log 尺度适配方法并记录。预期缺前提单独记录；意外代码错误在 Notebook 抛出，在 batch 保存 stage error 和已完成产物。网页只读这些产物，不运行科学计算。
